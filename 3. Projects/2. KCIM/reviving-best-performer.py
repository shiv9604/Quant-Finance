...
expected_filename = f"nifty_1min_{START_DATE.strftime('%Y%m%d')}_to_{END_DATE.strftime('%Y%m%d')}.csv"
...
if not windows_df.empty:
    ...
    # Save results
    windows_df.to_csv('intraday_expansion_windows.csv', index=False)
...
BASE_PATH = "./nifty_options_raw/nifty_data/nifty_options"
    ...
    '''
        ...
        if consolidation_exit and len(pnl_trail) >= 6:
            ...
            if last_consol_check is None or (t_dt - pd.Timestamp(f"2000-01-01 {last_consol_check}")).seconds >= 60:
                ...
                # Better — only fire if rebuild is meaningful (e.g. 2+ points)
                rebuild_pts = combined_now - combined_5m
                premium_rebuilding = (
                    rebuild_pts >= 2.0 and           # rebuilt at least 2 points
                    combined_now > combined_mid       # consistently rising
                )
...
'''
BACKTEST_START_5 = pd.Timestamp("2023-10-01")
BACKTEST_END_5   = pd.Timestamp("2024-10-31") 
SIGNAL_SCAN_FROM = datetime.time(10, 00)
SIGNAL_SCAN_TO   = datetime.time(11, 00)
LOT_SIZE_5       = 65
NUM_LOTS         = 4
...
if not trades_df_5.empty:
    ...
    # Save
    trades_df_5.to_csv("backtest_trades_step5.csv", index=False)
    print(f"\n✅ Trade log saved to backtest_trades_step5.csv")
    
    # Save minute by minute trail as separate CSV
    trail_df = pd.DataFrame(trails_5)
    trail_df.to_csv("backtest_pnl_trail.csv", index=False)
    print(f"✅ P&L trail saved to backtest_pnl_trail.csv")
    print(f"   {len(trail_df):,} rows — every minute of every trade")
else:
...
if not trades_df_6.empty:
    ...
    # Save
    trades_df_6.to_csv("backtest_trades_step6_volshort.csv", index=False)
    print(f"\n✅ Trade log saved to backtest_trades_step6_volshort.csv")

    trail_df_6 = pd.DataFrame(trails_6)
    trail_df_6.to_csv("backtest_pnl_trail_step6_volshort.csv", index=False)
    print(f"✅ P&L trail saved to backtest_pnl_trail_step6_volshort.csv")
    print(f"   {len(trail_df_6):,} rows — every minute of every trade")
